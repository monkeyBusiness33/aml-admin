...
function alterBandFieldNumbering(form_num) {
  ...
  let band_pricing_fields = document.querySelectorAll(`#collapseBandAccordion-${form_num} .band-discount-amount:not(.existing) input`)
  ...
  band_pricing_fields.forEach(field => {
    field.id = `id_new-pricing-0-band_discount_amount-additional-0-${i}`
    field.name = `new-pricing-0-band_discount_amount-additional-0-${i}`
    i++
  })
