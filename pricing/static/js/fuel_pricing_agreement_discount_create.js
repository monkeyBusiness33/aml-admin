...
function revertToDefault() {
  ...
  let percentage_toggle = document.querySelector(`#id_new-pricing-${form_num}-is_percentage`)
  let band_rows = document.querySelectorAll(`#collapseBandAccordion-${form_num} .band-row`)
  let band_pricing_field = document.querySelector(`#id_new-pricing-${form_num}-band_discount_amount`)
  let band_percentage_field = document.querySelector(`#id_new-pricing-${form_num}-band_discount_percentage`)
  let form_pricing_field = document.querySelector(`#id_new-pricing-${form_num}-pricing_discount_amount`)
  let form_percentage_field = document.querySelector(`#id_new-pricing-${form_num}-pricing_discount_percentage`)
  ...
  band_percentage_field.required = false
  band_percentage_field.parentElement.parentElement.classList.add('d-none')

  if (percentage_toggle.checked) {
    form_percentage_field.required = true
    form_percentage_field.parentElement.parentElement.classList.remove('d-none')
    form_pricing_field.required = false
    form_pricing_field.parentElement.parentElement.classList.add('d-none')
  } else {
    form_percentage_field.required = false
    form_percentage_field.parentElement.parentElement.classList.add('d-none')
...
function alterRequiredAttribute(form_num, action) {
  ...
  let amount_fields = document.querySelectorAll(
    `#collapseBandAccordion-${form_num} .band-row [name*="discount_amount"]`)
  let percentage_fields = document.querySelectorAll(
    `#collapseBandAccordion-${form_num} .band-row [name*="discount_percentage"]`)
  let percentage_toggle = document.querySelector(
    `#id_new-pricing-${form_num}-is_percentage`)

  amount_fields.forEach(field => {
    if (action == 'add' && !percentage_toggle.checked) {
      field.required = true
      if (field.previousElementSibling) {
        field.previousElementSibling.classList.add('required')
      }
...
function alterAmountField(form_num, action) {
  ...
  if (action == 'add') {
    if (percentage_toggle.checked) {
      ...
      band_percentage_field.required = true
      band_percentage_field.parentElement.parentElement.classList.remove('d-none')
      if (band_percentage_field.value == '') {
        band_percentage_field.value = form_pricing_field.value
      }
    } else {
      ...
      band_percentage_field.required = false
      band_percentage_field.parentElement.parentElement.classList.add('d-none')
      band_percentage_field.value == ''
    }

    form_pricing_field.value = ''
    form_pricing_field.required = false
    form_pricing_field.parentElement.parentElement.classList.add('d-none')
    form_percentage_field.value = ''
    form_percentage_field.required = false
    form_percentage_field.parentElement.parentElement.classList.add('d-none')
  } else {
    band_pricing_field.required = false
    ...
    if (percentage_toggle.checked) {
      form_percentage_field.value = band_pricing_field.value
      form_percentage_field.required = true
      form_percentage_field.parentElement.parentElement.classList.remove('d-none')
    } else {
...
function alterBandInput() {
  ...
  let band_pricing_div = document.querySelector(`#collapseBandAccordion-${form_num} .band-discount-amount`)
...
function alterBandFieldNumbering(form_num) {
  let band_start_fields = document.querySelectorAll(`#collapseBandAccordion-${form_num} .band-start input`)
  let band_end_fields = document.querySelectorAll(`#collapseBandAccordion-${form_num} .band-end input`)
  let band_pricing_fields = document.querySelectorAll(`#collapseBandAccordion-${form_num} .band-discount-amount input`)
  let band_percentage_fields = document.querySelectorAll(`#collapseBandAccordion-${form_num} .band-discount-percentage input`)
  ...
  if (band_start_fields.length > 1) {
    ...
    band_pricing_fields.forEach(field => {
      if (i > 0) {
        field.id = `id_new-pricing-${form_num}-band_discount_amount-additional-${form_num}-${i}`
        field.name = `new-pricing-${form_num}-band_discount_amount-additional-${form_num}-${i}`
      }
      i++
    })

    i = 0
    band_percentage_fields.forEach(field => {
      if (i > 0) {
        field.id = `id_new-pricing-${form_num}-band_discount_percentage-additional-${form_num}-${i}`
        field.name = `new-pricing-${form_num}-band_discount_percentage-additional-${form_num}-${i}`
      }
      i++
    })
  }
...
function addMissingRows() {
  ...
  all_missing_forms.forEach(index => {
    ...
    let destination_type = document.querySelector(`[name*="-${index}-destination_type"]`)
    let ipa = document.querySelector(`[name*="-${index}-ipa"]`)
    let pricing_unit = document.querySelector(`[name*="-${index}-pricing_discount_unit"]`)
    ...
    let discount_type = document.querySelector(`[name*="-${index}-is_percentage"]`)
    ...
    let new_pricing_unit = form_sections[k].querySelector(`[name*="pricing_discount_unit"]`)
    ...
    let new_discount_type = form_sections[k].querySelector(`[name*="is_percentage"]`)
    ...
    new_discount_type.checked = discount_type.checked;
    new_discount_type.click();
    ...
    if ($(pricing_unit).select2('data').length) {
...
function switchDiscountType() {
  ...
  let band_uom_field = document.querySelector(`#id_new-pricing-${form_num}-band_uom`)
  let band_rows = document.querySelectorAll(`#collapseBandAccordion-${form_num} .band-row`)
  let band_pricing_fields = document.querySelectorAll(`[id*="id_new-pricing-${form_num}-band_discount_amount"`)
  let band_percentage_fields = document.querySelectorAll(`[id*="id_new-pricing-${form_num}-band_discount_percentage"`)
  let form_discount_unit_field = document.querySelector(`#id_new-pricing-${form_num}-pricing_discount_unit`)
  let form_pricing_field = document.querySelector(`#id_new-pricing-${form_num}-pricing_discount_amount`)
  let form_percentage_field = document.querySelector(`#id_new-pricing-${form_num}-pricing_discount_percentage`)
  let isPercentage = this.firstChild.checked;
  let selected_data = $(band_uom_field).select2('data')[0].text
  let hasBands = selected_data !== "" && selected_data !== "---------" || band_rows.length > 1;

  if (isPercentage) {
    form_discount_unit_field.required = false
    form_discount_unit_field.parentElement.parentElement.classList.add('d-none')
    band_percentage_fields[0].previousElementSibling.classList.add('required')
    band_pricing_fields[0].previousElementSibling.classList.remove('required')
    ...
    form_discount_unit_field.required = true
    form_discount_unit_field.parentElement.parentElement.classList.remove('d-none')
    band_percentage_fields[0].previousElementSibling.classList.remove('required')
    band_pricing_fields[0].previousElementSibling.classList.add('required')
