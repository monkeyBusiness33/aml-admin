...
function addAllEventListeners(){
    ...
    // Supplier exchange rate application result display
    let fuelFields = $('[name$="-fuel"]');
    let nativeUnitFields = $('label[for*="pricing_native_unit"]');
    let nativeAmountFields = $('[name*="differential_pricing_unit"]');
...
function revertToDefault(){
    ...
    let band_pricing_field = document.querySelector(`#id_new-pricing-${form_num}-band_pricing_native_amount`)
    let form_pricing_field = document.querySelector(`#id_new-pricing-${form_num}-pricing_native_amount`)
...
function alterBandFieldNumbering(form_num){
    ...
    if(band_start_fields.length > 1){
        ...
        band_pricing_fields.forEach(field => {
            if(i > 0){
                field.id = `id_new-pricing-${form_num}-band_pricing_native_amount-additional-${form_num}-${i}`
                field.name = `new-pricing-${form_num}-band_pricing_native_amount-additional-${form_num}-${i}`
            }
            i++
        })
    }
...
post_button.addEventListener('click', function(e){
    ...
    let missing_main_form_numbers = []
    let missing_main_form_data = []
    let missing_sub_form_numbers = []
    let missing_sub_form_data = []
    all_missing_forms = []
    let form_valid_for_missing = []
    ...
    if(visible_forms.length == 1){
        ...
        for (let main_iterator = 0; main_iterator < forms_to_process.length; main_iterator++){
            ...
            let valid_from = document.querySelector(`[name*="-${index}-valid_from_date"]`)
            let valid_to = document.querySelector(`[name*="-${index}-valid_to_date"]`)
            ...
            let data_list = [location.value, fuel.value, delivery_method.value, flight_type.value, ipa.value, valid_from.value, valid_to.value]
            ...
            for (let sub_iterator = 1; sub_iterator < forms_to_process.length; sub_iterator++){
                ...
                let sub_valid_from = document.querySelector(`[name*="-${sub_index}-valid_from_date"]`)
                let sub_valid_to = document.querySelector(`[name*="-${sub_index}-valid_to_date"]`)

                let sub_data_list = [sub_location.value, sub_fuel.value, sub_delivery_method.value, sub_flight_type.value, sub_ipa.value, sub_valid_from.value, sub_valid_to.value]
    ...
    if(all_missing_forms.length > 0 && !missing_key_fields){
        ...
        modal_text.innerHTML = `The pricing details entered appears to be incomplete, with the following entries missing:<br>
 ...
 function addMissingRows(){
    ...
    all_missing_forms.forEach(index => {
        ...
        let valid_from = document.querySelector(`[name*="-${index}-valid_from_date"]`)
        let valid_to = document.querySelector(`[name*="-${index}-valid_to_date"]`)
        let pricing_unit = document.querySelector(`[name*="-${index}-pricing_native_unit"]`)
        ...
        let new_valid_from = form_sections[k].querySelector(`[name*="valid_from_date"]`)
        let new_valid_to = form_sections[k].querySelector(`[name*="valid_to_date"]`)
        let new_pricing_unit = form_sections[k].querySelector(`[name*="pricing_native_unit"]`)
        ...
        $(new_valid_from).val($(valid_from).val()).trigger('change')
        $(new_valid_to).val($(valid_to).val()).trigger('change')
        $(new_pricing_unit).val($(pricing_unit).val()).trigger('change')
