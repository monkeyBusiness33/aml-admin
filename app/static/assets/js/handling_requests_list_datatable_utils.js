                ...
                if (item.visible) {
                    if (item.searchable) {
                        ...
                        if ('choices' in item && item.choices) {
                            ...
                            // Convert value to the array
                            if (item.initialSearchValue) {
                                // let valuesList = item.initialSearchValue.split(',');
                                let valuesList = item.initialSearchValue.split(',').map(Number);
                                if (valuesList.length > 1) {
                                    item.initialSearchValue = valuesList
                                }
                            }

                            // Convert null values to empty string to make it compatible with array.includes()
                            if (item.initialSearchValue === null) {
                                item.initialSearchValue = ''
                            }

                            // Process 'status' column differently
                            let select = ''
                            if (item.name === 'status' || item.name === 'status_code') {
                                select = $('<select multiple data-index="' + index.toString() + '"><option value=""></option></select>');
                                $(item.choices).each(function (index, choice) {
                                    if (item.initialSearchValue.includes(choice[0])) {
                                            var option = new Option(choice[1], choice[0], true, true);
                                        } else {
                                            var option = new Option(choice[1], choice[0], false, false);
                                        }
                                    select.append(option);
                                });
                            } else {
                                // See: https://www.datatables.net/examples/api/multi_filter_select.html
                                select = $('<select data-index="' + index.toString() + '"><option value=""></option></select>');
                                $(item.choices).each(function (index, choice) {
                                    var option = $("<option>").attr('value', choice[0]).text(choice[1], true, true);
                                    if (item.initialSearchValue.includes(choice[0])) {
