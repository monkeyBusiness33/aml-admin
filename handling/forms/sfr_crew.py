...
from handling.models import HandlingRequestCrew
...
class HandlingRequestCrewForm(forms.ModelForm):

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.handling_request = getattr(self.instance, 'handling_request', None)
    ...
    class Meta:
        ...
        fields = ['person', 'position', 'can_update_mission', 'is_primary_contact', ]
        widgets = {
...
class HandlingRequestCrewBaseFormSet(BaseModelFormSet):
    ...
    def clean(self):
        ...
        arrival_date = self.handling_request.get_eta_date()
        departure_date = self.handling_request.get_etd_date()
        ...
        for form in valid_forms:
            ...
            if person:
                ...
                crew_member = HandlingRequestCrew.objects.filter(exclude_self_q, exclude_instances_to_delete_q,
                ...
                overlapped_sfr, overlapped_missions = get_person_active_crews(
                    ...
                    exclude_sfr=self.handling_request,
                )
                if overlapped_sfr:
        ...
        cleaned_data = super(HandlingRequestCrewBaseFormSet, self).clean()
...
HandlingRequestCrewFormSet = modelformset_factory(
    HandlingRequestCrew,
    extra=10,
    can_delete=True,
    form=HandlingRequestCrewForm,
    formset=HandlingRequestCrewBaseFormSet,
    fields=['person', 'position', 'can_update_mission', 'is_primary_contact', ]
)
